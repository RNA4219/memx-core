...
外部 issue を「外界の窓口」として workx / memx-core に接続する。ただし tracker を正本にしない。
...
- `entity_link`: issue と workx task のリンク
- `sync_event`: 同期イベント履歴
...
| **linking** | `tracker:issue:*:*` と `workx:task:local:*` のリンク（role: primary/related/duplicate/blocks） |
...
2. issue と `workx task` を entity_link できる
3. inbound / outbound の sync_event を追跡できる
4. `workx context build` が tracker snapshot を optional input として使える
5. tracker を見失っても `workx` と `memx-core` だけで task 継続は可能
